...
protected:
	...
	void ensureDataLoaded(NSCloner::Ptr &ns, const RdxContext &ctx);
	...
	NSCloner::Ptr getNamespace(string_view nsName, const RdxContext &ctx);
	NSCloner::Ptr getNamespaceNoThrow(string_view nsName, const RdxContext &ctx);

	std::vector<std::pair<string, NSCloner::Ptr>> getNamespaces(const RdxContext &ctx);
	std::vector<string> getNamespacesNames(const RdxContext &ctx);

	fast_hash_map<string, NSCloner::Ptr, nocase_hash_str, nocase_equal_str> namespaces_;

...
const std::string JsonSchemaChecker::kRootTypeName_ = "root";

JsonSchemaChecker::JsonSchemaChecker(const std::string& json) {
	Error err = createTypeTable(json);
	if (!err.ok()) throw err;
}

Error JsonSchemaChecker::Init(const std::string& json) { return createTypeTable(json); }
...
Error JsonSchemaChecker::createTypeTable(const std::string& json) {
	...
	try {
		createType(root, kRootTypeName_);
	} catch (const Error& e) {
		return e;
	}
...
Error JsonSchemaChecker::Check(gason::JsonNode node) {
	...
	auto indxIt = indexes_.find(kRootTypeName_);
	if (indxIt == indexes_.end()) return Error(errParseJson, "Type '%s' not found.", kRootTypeName_);
	int nType = indxIt->second;
	std::string path;
	path.reserve(512);
	return checkScheme(node, nType, path, kRootTypeName_);
}
	...
	for (const auto& elem : node) {
		...
		if (elem.value.getTag() == gason::JSON_OBJECT) {
			if (descr.subElementsTable[subElemIndex->second].second.typeName != "any") {
				// auto typeIndex = descr.subElementsTable[subElemIndex->second].second.typeIndex;
				//				unsigned int pathLen = path.size();
				//				path += ".";
				//				path += descr.subElementsTable[subElemIndex->second].first;
